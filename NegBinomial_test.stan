...
model {
  ...
  for (i in 1:N) {
    if (Y[i] == 0) {
      ...
      target += bernoulli_lpmf(0 | p_zero[i]) + neg_binomial_2_lpmf(0 | mu[i], phi_nb);  // zero count with negative binomial
    } else {
      target += bernoulli_lpmf(0 | p_zero[i]);                              // Zero-inflation: non-zero counts
      target += neg_binomial_2_lpmf(Y[i] | mu[i], phi_nb);                 // negative binomial likelihood for non-zero counts
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:N) {
    if (Y[i] == 0) {
      ...
      log_lik[i] = bernoulli_lpmf(0 | p_zero[i]) + neg_binomial_2_lpmf(Y[i] | mu[i], phi_nb);
    }
  }
}
